//! Dispatcher binary: runs one search exercise's solution against a
//! fixture and target with tracing enabled, printing the recorded events
//! as JSON to stdout. Unlike the sort module's trace binaries, this one
//! always expects a 3rd argument (the target) — every search exercise has
//! one.
//!
//! Run as a fresh subprocess (`cargo run -p exercises-search --bin
//! exercises-search-trace -- <name> <fixture-json> <target>`) so it always
...
fn main() {
    ...
    match name.as_str() {
        "linear" => {
            exercises_search::linear_search(&arr, target);
        }
        "binary" => {
            exercises_search::binary_search(&arr, target);
        }
        other => {
