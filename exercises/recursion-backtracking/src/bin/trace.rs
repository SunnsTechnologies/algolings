...
//!
//! `recursion_basics` bundles two functions (factorial, fibonacci) into
//! one exercise — enable() is called once and BOTH run before
//! take_events(), so their traces concatenate into a single combined
//! trace, the same way doubly_pop's dispatcher already chains
//! pop_front then pop_back.
//!
//! n_queens uses `target` (not `fixture`) for its board size — `fixture`
//! has no natural meaning for a single scalar input, and `target` already
//! exists for exactly that (search exercises' target value).
//!
//! Run as a fresh subprocess (`cargo run -p exercises-recursion-backtracking
//! --bin exercises-recursion-backtracking-trace -- <name> <fixture-json>
//! [target]`) so it always reflects the CURRENT on-disk solution code.

use algolings_trace::{enable, take_events};
use exercises_recursion_backtracking::{
