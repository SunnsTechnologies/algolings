...
/// colorblind users.
pub fn draw_frame(f: &mut Frame, frame: &PlayerFrame, exercise_name: &str, auto_play: bool) {
...
pub fn run_interactive(
    ...
    let result = (|| -> std::io::Result<()> {
        loop {
            terminal.draw(|f| draw_frame(f, &player.current_frame(), exercise_name, auto_play))?;
...
mod tests {
    ...
    fn draws_the_array_with_asterisk_on_highlighted_positions() {
        ...
        terminal
            .draw(|frm| draw_frame(frm, &f, "bubble_sort", false))
            .unwrap();
    ...
    fn controls_reflect_auto_play_state() {
        ...
        terminal
            .draw(|frm| draw_frame(frm, &f, "bubble_sort", true))
            .unwrap();
